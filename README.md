...
![手机票档文字生成点选项与预算](docs/images/phone-tiers.png)

![一项任务纳入全部已确认场次并自动计算预算](docs/images/all-sessions.png)
...
2. 在具体官方活动页点击“读取活动信息”。大麦详情页可识别标题、场馆、明确列出的全部场次、单笔限购及公开价格范围；其他平台在页面含标准公开活动标记时，可带入明确的日期、票档名称和单价。候票台把**当前来源明确列出的所有场次**一起加入一项演出任务，无需逐场选择；有逐档名称与单价时会列成卡片，你按偏好点选票档，再选人数，预算自动按最高已选单价和所选费用余量生成。旧版单场次任务仍可读取。只有价格范围而无逐档价格时，界面会明确提示，**不会**把范围上下限冒充票档；该类项目须在原平台查看准确票档。可以把手机文字识别出的“票档名称 ¥单价”逐行粘贴，生成可点选项；粘贴内容仅在本机处理，不读取账号和证件。软件不会把日期范围猜成具体日期，也不自动推断不明确的所在地时区。
...
例如深圳站邓紫棋项目的[大麦公开页](https://detail.damai.cn/item.htm?id=1052123401368)在退票规则中明确列出 9 个 19:00 场次，但网页购票状态要求使用大麦 App，且只给出 ¥380–¥1680 的范围。候票台可把**能核实的 9 个场次全部纳入一个任务**；逐档票名与价格须从 App 内对应场次核对后导入，不能凭范围补造。选定票档后，预算按最高已选单价 × 人数及所选 0%、10% 或 20% 余量计算；它是可调整的个人上限，不是官方实收费用。该项目页面还提示不支持自主选座，因此不能提供真实座位点选。不同场次的票档及价格可能不同，导入文字不能证明各场次都在售。
