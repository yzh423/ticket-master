...
1. 打开候票台即可在“发现演出”输入歌手或活动名，在大麦官方搜索结果中打开具体项目；也可直接粘贴大麦活动链接。Windows 桌面版会从公开详情页带入标题、日期范围、场馆和官方链接。若搜索出现验证码，需在大麦页面自行完成。网页版只负责打开官方搜索，不能跨网站读取详情。
2. 核对并填写固定演出场次、人数、含费用的总预算，以及按优先顺序排列的可接受票档。页面只有日期范围时，软件不会猜测具体场次。也可以直接“新建任务”，继续手动录入其他平台。
...
内置网页仅供桌面版使用：在“发现演出”打开大麦搜索或详情，候票台读取当前公开项目资料并让你确认是否填入任务。再次搜索复用发现窗口。选择已建任务后点击“内置官方网页”，官方页面与本场人数、预算、票档并排显示。同一任务再次点击只会聚焦原窗口，不会主动刷新排队会话。发现窗口与购票窗口互斥，切换前须关闭当前窗口。部分项目仅支持官方 App 下单，第三方登录、支付弹窗或平台限制也可能使内置网页无法继续；此时使用官方 App 或系统浏览器，并在官方页面核对订单。系统浏览器与内置网页的登录会话不共享。

## 平台与数据边界

大麦是首版优先核对的官方渠道，也是目前唯一支持公开详情页辅助带入的平台。其他平台可以建立任务、记录公告与打开已识别的 HTTPS 官方域名，但平台图标不代表已接入搜索、实时库存或交易接口。每场演出的实名、限购、候补和补票规则均以当场官方公告为准。候票台不会推测“整点回流”，也没有实测数据支持固定的成功率提升百分比。
