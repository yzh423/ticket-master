...
内置网页仅供桌面版使用：“发现演出”的官网直接显示在主界面，重复搜索复用同一平台会话，切换平台时使用独立会话。选择已建任务后点击“内置官方网页”，购票页面与本场人数、预算、票档并排显示在专用工作区；同一任务再次点击只会聚焦原窗口，不会主动刷新排队会话。进入排队或结账后，请遵守平台规则，不要随意切换、关闭或刷新当前页面。部分项目仅支持官方 App 下单，登录、支付弹窗或平台限制也可能使内置网页无法继续；此时使用官方 App 或系统浏览器，并在官方页面核对订单。系统浏览器与内置网页的登录会话不共享。
