...
Vite 会自动打开浏览器；如果没有自动打开，请访问终端打印的 `http://127.0.0.1:端口/` 地址。请保持这个终端窗口运行。`pnpm web` 只在本机启动网页服务，GitHub 仓库地址本身不是网页应用地址。网页版本会在新标签页打开经过域名检查的官方购票入口；任务保存在**当前浏览器的本站本地存储**，与桌面版数据不互通。清除站点数据、换浏览器或换电脑会失去这些记录，重要信息请另行备份。网页版本没有内置浏览器会话、Windows 桌面提醒和 Android USB 功能。

## 如何使用

1. 新建任务，填写官方项目入口、固定演出场次、人数、含费用的总预算，以及按优先顺序排列的可接受票档。
2. 从官方公告录入优先购、公开销售、正式补票、候补或邀请机会，写明资格、开始与截止时间、活动所在地时区和来源。
3. 开售前核对账号、实名观演人、购买资格、付款方式、设备与平台通知。桌面版运行时会针对已录入机会发出阶段提醒；关闭后不会继续提醒。
4. 在官方页面人工判断票档与最终总价。状态不明时不要误认为售罄；已有有效订单时优先完成付款，不重复提交。
5. 分别记录排队、待支付、已付款待出票和已出票等结果，并保留官方确认依据。
